...
st.set_page_config(page_title="AInterviewer", page_icon="💬")
st.title("💬 AInterviewer")
...
if not st.session_state.setup_complete: 
    ...
    with col1:
        ...
        try:
            default_index = level_options.index(st.session_state["level"])
        except ValueError:
            default_index = 0
    ...
    if st.button("Start Interview", type="primary", on_click=complete_setup):
...
if st.session_state.setup_complete and not st.session_state.feedback_shown and not st.session_state.chat_complete:
    ...
    # Initialize OpenAI Client (use unified secret key `OPEN_AI_KEY`)
    ...
    # Hanldle user input and OpenAI response
    ...
    if st.session_state.user_message_count < 5:
            if prompt := st.chat_input("Ask me anything!", max_chars=1000):
                ...
                # Always attempt to produce an assistant reply for each user message
...
if st.session_state.chat_complete and not st.session_state.feedback_shown:
    if st.button("Get Feedback", on_click=show_feedback):
        st.write("Feedback button clicked. Showing feedback...")
